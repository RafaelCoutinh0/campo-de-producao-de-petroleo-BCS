...
# Função de inércia adaptativa
def adaptive_inertia(w_max, w_min, current_iter, max_iter):
    return w_max - (w_max - w_min) * (current_iter / max_iter)

# Configurações do PSO com inércia adaptativa
options_pso = {'c1': 0.5, 'c2': 0.3, 'w': 0.9}  # w inicial
...
# Função para otimização com inércia adaptativa
def optimize_with_adaptive_inertia(optimizer, objective_function, iters):
    for i in range(iters):
        # Atualiza a inércia
        options_pso['w'] = adaptive_inertia(w_max=0.9, w_min=0.4, current_iter=i, max_iter=iters)
        
        # Executa uma iteração do PSO
        optimizer.optimize(objective_function, iters=1)
    
    return optimizer

# Executar o PSO com inércia adaptativa
start_time_pso = time.time()
optimizer_pso = optimize_with_adaptive_inertia(optimizer_pso, optimization_objective, iters=10)
best_obj_value_pso, best_u_pso = optimizer_pso.optimize(optimization_objective, iters=1)
...
de_execution_time = end_time_de - start_time_pso
...
print(f"Tempo de execução PSODE: {de_execution_time:.2f} segundos")